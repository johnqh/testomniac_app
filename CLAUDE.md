...
- **testomniac_lib** — Business logic library with `useHistoriesManager` hook; primary integration point for this app
- **testomniac_api** — Backend server that this app communicates with (watch the port mismatch!)
- **testomniac_app_rn** — React Native counterpart of this web app; shares testomniac_client, testomniac_lib, and testomniac_types
...
- API URL: `.env` defaults to `localhost:8022` to match the API server (`testomniac_api`) -- verify `VITE_API_URL` matches your running API if using a different port
